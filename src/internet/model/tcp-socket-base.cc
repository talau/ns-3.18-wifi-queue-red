...
TcpSocketBase::DoConnect (void)
{
  ...
  if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK || m_state == CLOSE_WAIT)
    { // send a SYN packet and change state into SYN_SENT
      ...
      // ReTxTimeout, here fix?
      //      Simulator::Schedule (m_rto, &TcpSocketBase::ReTxTimeout, this);
    }
...
TcpSocketBase::CloseAndNotify (void)
{
  ...
  //  std::cout << "\t-----------------CloseAndNotify()" << std::endl;
      ...
      if (tcpHeader.GetAckNumber () < m_nextTxSequence && packet->GetSize() == 0)
        {
          ...
          // register the number of tripleacks
          if (m_dupAckCount >= 3)
            {
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (m_retxEvent.IsExpired () )
    { // Schedule retransmit
      ...
      //      if (!m_connected)
      //        std::cout << m_rtt->RetransmitTimeout().GetSeconds() << std::endl;
    }
...
TcpSocketBase::ReTxTimeout ()
{
  /*
  if (!m_connected) {
    std::cout << GetNode()->GetId() << "; not connected\n";
    CloseAndNotify();
  ...
  */
  ...
  if (m_state == CLOSED || m_state == TIME_WAIT)
    {
      //  std::cout << "retxtimeout 1\n";
      return;
    }
  // If all data are received (non-closing socket and nothing to send), just return
  if (m_state <= ESTABLISHED && m_txBuffer.HeadSequence () >= m_highTxMark)
    {
      //  std::cout << "retxtimeout 2\n";
      return;
    }
...
{
  //  std::cout << "DoRetransmit" << std::endl;

  NS_LOG_FUNCTION (this);
  // Retransmit SYN packet
  if (m_state == SYN_SENT)
    {
      //      std::cout << "SYN_SENT, but retransmit" << std::endl;
      if (m_cnCount > 0)
        {
          SendEmptyPacket (TcpHeader::SYN);
        }
