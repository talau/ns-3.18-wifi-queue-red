...
void BulkSendApplication::ConnectionSucceeded (Ptr<Socket> socket)
{
  ...
  m_connected = true;
  std::cout << "\t++++++++ Connection succeed from node ->" << GetNode()->GetId() << std::endl;
  SendData ();
}
...
void BulkSendApplication::ConnectionFailed (Ptr<Socket> socket)
{
  ...
  //  std::cout << "ConnectionFailed()\n";
  //  if (!m_connected)
  //    Simulator::ScheduleNow (&BulkSendApplication::StartApplication, this);
}
...
uint32_t
BulkSendApplication::GetTotalTx()
{
  return m_totBytes;
}
